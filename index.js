...
try{
if (sessionnye == "multi") {
let session = require("@adiwajshing/baileys").useMultiFileAuthState
let { state, saveCreds } = await session("./sessionye")
var dataSesi = state
var saveSesi = saveCreds
} else if (sessionnye == "single") {
let session = require("@adiwajshing/baileys").useSingleFileAuthState
let { state, saveState } = await session("./sessionye/session.json")
var dataSesi = state
var saveSesi = saveState
} else {
let session = require("@adiwajshing/baileys").useMultiFileAuthState
let { state, saveCreds } = await session("./sessionye")
var dataSesi = state
...
} catch {console.log("Type session tidak di temukan")}
...
const zyren = makeWASocket(connectionOptions)
//=================================================//
store.bind(zyren.ev)
//=================================================//
...
else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); process.exit() }
...
else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); await connectToWhatsApp() }
...
//=================================================//
zyren.ev.on('creds.update', saveSesi)
return zyren
}
connectToWhatsApp()
//=================================================//
